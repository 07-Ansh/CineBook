...
export const createOrder = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const seatCheck = await client.query(
      `SELECT id, is_booked, (held_until IS NOT NULL AND held_until > NOW()) AS is_held
       FROM seats
    ...
    const unavailableSeats = seatCheck.rows.filter(
      seat => seat.is_booked || seat.is_held
    );
    ...
    try {
      ...
      return res.status(502).json({ error: 'Payment gateway error. Please verify your Razorpay API keys.' });
    }
